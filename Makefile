...
DEPEND = Makefile \
	...
	main.c main.h keyboard.c keyboard.h \
	printf.c 

OBJ=$(KERNEL_OBJ) gdt.o pic.o idt.o isr.o isr_c.o mmu.o sched.o tss.o screen.o game.o perro.o jugador.o game_isr.o printf.o keyboard.o main.o
...
TASKSBINS=$(TASKIDLE) $(TASKA1) $(TASKA2) $(TASKB1) $(TASKB2)
...
taskA1.tsko: tareaA1.c
	$(CC) $(CFLAGS) -c -o $@ $^
taskA2.tsko: tareaA2.c
	$(CC) $(CFLAGS) -c -o $@ $^
taskB1.tsko: tareaB1.c
	$(CC) $(CFLAGS) -c -o $@ $^
taskB2.tsko: tareaB2.c
	$(CC) $(CFLAGS) -c -o $@ $^
...
tasks: $(TASKSBINS)
	...
	$(QUIET) dd if=$(TASKA1)   bs=1 count=4K of=$(TASKS) seek=0K  status=noxfer > /dev/null 2>&1
	$(QUIET) dd if=$(TASKA2)   bs=1 count=4K of=$(TASKS) seek=4K  status=noxfer > /dev/null 2>&1
	$(QUIET) dd if=$(TASKB1)   bs=1 count=4K of=$(TASKS) seek=8K status=noxfer > /dev/null 2>&1
	$(QUIET) dd if=$(TASKB2)   bs=1 count=4K of=$(TASKS) seek=12K status=noxfer > /dev/null 2>&1
